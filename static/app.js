/* ═══════════════════════════════════════════════════════════════
   ...
   WEBUI_MODE is set by the server at runtime via window.WEBUI_MODE
   ═══════════════════════════════════════════════════════════════ */
...
function applyWebUiMode() {
  ...
  document.body.classList.add(`mode-${WEBUI_MODE}`);
}

function pruneInactiveModeDom() {
  const inactiveSelector = WEBUI_MODE === 'hub' ? '.spoke-only' : '.hub-only';
  document.querySelectorAll(inactiveSelector).forEach((element) => element.remove());
}

applyWebUiMode();
pruneInactiveModeDom();
...
function showToast(message, level = 'ok') {
  ...
  const kind = String(level || 'ok').toLowerCase();
  const cls = kind === 'err' || kind === 'error' || kind === 'warn' || kind === 'warning' ? 'error' : 'success';
...
function runModeSource(mode, source) {
  if (WEBUI_MODE !== mode) return;
  (0, eval)(source);
}

/* ── SPOKE CODE (active when WEBUI_MODE === 'spoke') ─────────── */
const SPOKE_SOURCE = [
  "const FLAG_ORDER = [",
  "  'kill_switch',",
  "  'dns_fail',",
...
runModeSource('spoke', SPOKE_SOURCE);

/* ── HUB CODE (active when WEBUI_MODE === 'hub') ─────────────── */
const HUB_SOURCE = [
  "\"use strict\";",
  "",
